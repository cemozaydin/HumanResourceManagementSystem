...
public interface CandidateLanguageDao extends JpaRepository<CandidateLanguage, Integer> {
    ...
    @Query(value = "SELECT new kodlamaio.hrms.entities.dtos.CandidateLanguageDetailsDto" +
            "(cl.id, ca.id, lng.name, cl.level) " +
            "FROM Candidate ca " +
            "INNER JOIN ca.candidateLanguages cl " +
            "INNER JOIN Language lng ON cl.languageId = lng.id")
    List<CandidateLanguageDetailsDto> getAllCandidateLanguageDetails();
    ...
    @Query(value = "SELECT new kodlamaio.hrms.entities.dtos.CandidateLanguageDetailsDto" +
            ...
            "INNER JOIN ca.candidateLanguages cl " +
            "INNER JOIN Language lng ON cl.languageId = lng.id")
    List<CandidateLanguageDetailsDto> getCandidateLanguageDetailsByCandidateId(int id);
